...
async function restoreOptions() {
    ...
    if (gitlabCustomDomainKey in res) {
        const inputEl = document.querySelector(gitlabCustomDomain) as HTMLInputElement;
        if (inputEl) {
...
async function saveOptions(e: Event) {
    ...
    items[gitlabCustomDomainKey] = (document.querySelector(gitlabCustomDomain) as HTMLInputElement).value
    await browser.storage.local.set(items).catch(reason => console.error(reason));
