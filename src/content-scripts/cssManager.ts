...
const mrViewSelector = (attrName: string, uuid: string): string =>
	`[${attrName}="${uuid}"] > .diff-grid .diff-tr`;

const commitViewLeftSelector = (attrName: string, uuid: string): string =>
	`[${attrName}="${uuid}"] td.left-side`;
const commitViewRightSelector = (attrName: string, uuid: string): string =>
	`[${attrName}="${uuid}"] td.right-side`;
...
function adjustRules(attrName: string, uuid: string, perc: number) {
	...
	for (const cssRulesKey in styleSheet.cssRules) {
...
function ensureCssSheet(): CSSStyleSheet {
	...
	CSS_STYLE_SHEET = Array.from(document.styleSheets).find(
		(ss) =>
