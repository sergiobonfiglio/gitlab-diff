...
function addMouseListener(triggerElement: HTMLElement, container: HTMLElement, onChangePerc: (perc: number) => void): void {
...
`)
    ...
    parents.forEach((pElement) => {
        ...
        if (parent && !parent.hasAttribute(MARKING_ATTR_NAME)) {
            ...
            const filePermissions = document.querySelector(`.diff-file:has([data-qol='${uuid}']) .file-header-content > small:last-child`)
            ...
            if (filePermissions) {
                ...
                const left = parseInt(sides[0])
                if (left === 0) {
                    onPercChange(MIN_HANDLE_POS)
                }
                const right = parseInt(sides[sides.length - 1]);
                if (right === 0) {
